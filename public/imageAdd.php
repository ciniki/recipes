...
function ciniki_recipes_imageAdd(&$ciniki) {
    ...
    $rc = ciniki_recipes_checkAccess($ciniki, $args['tnid'], 'ciniki.recipes.imageAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    } 
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.recipes.5', 'msg'=>'Unable to get a new UUID', 'err'=>$rc['err'])    );
    }
    $args['uuid'] = $rc['uuid'];
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.recipes.6', 'msg'=>'You already have an image with this name, please choose another name'));
    }


    if( $args['recipe_id'] <= 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.recipes.7', 'msg'=>'No recipe specified'));
    }
    ...
    return ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.recipes.image', $args, 0x07);
}
?>
